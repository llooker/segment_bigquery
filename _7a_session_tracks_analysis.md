...
**Looker Model** - These individual files can be integrated in the model file [tracks](bigquery_segment_tracks.model.lkml) as shown. Note that event_id is NOT the primary key for Tracks, so joins must use multiple identifiers on required fields, like anonymous_id, received_at, and event_id.  **Also event_id may be called id, just swap the name**
